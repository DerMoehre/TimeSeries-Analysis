    ...
    def update_graph(
        ...
        if trigger_id == "run-model-button":
            ...
            if not all(col in df.columns for col in ["ds", "y", "unique_id"]):
                return check_upload_data(transformed_data, "invalid data format")
            ...
            if not freq:
                return check_upload_data(
                    transformed_data, "Frequency parameter is required"
                )
            ...
            if not season:
                return check_upload_data(
                    transformed_data, "Season length parameter is required"
                )
            ...
            if not model:
                return check_upload_data(transformed_data, "invalid model selected")
        ...
        # Default empty graph
        check_upload_data(transformed_data, "Uploaded data")
